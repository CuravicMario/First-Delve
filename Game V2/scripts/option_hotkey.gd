...
func _input(event):
	if in_focus:
		if Input.is_action_just_pressed("click") or Input.is_action_just_released("interact"):
			AudioMenager.create_audio(SoundEffect.SOUND_EFFECT_TYPE.UI_SELECT)
			if !parent.is_remaping:
				parent.is_remaping = true
				parent.action_to_remap = action
				parent.button_to_remap = self
				label.button.text = "Press any key..."
...
func update_text():
	label.button.text = str(InputMap.action_get_events(action)[0].as_text_physical_keycode())
